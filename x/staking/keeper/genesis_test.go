...
func (suite *KeeperTestSuite) TestInitGenesis(t *testing.T) {
	addrs, _ := generateAddresses(suite.bankKeeper, suite.stakingKeeper, suite.ctx, 10)
	...
	suite.Require().NoError(testutil.FundModuleAccount(
		suite.bankKeeper,
		suite.ctx,
...
func (suite *KeeperTestSuite) TestInitGenesis_PoolsBalanceMismatch(t *testing.T) {
...
func (suite *KeeperTestSuite) TestInitGenesisLargeValidatorSet(t *testing.T) {
	size := 200
	addrs, _ := generateAddresses(suite.bankKeeper, suite.stakingKeeper, suite.ctx, size)
