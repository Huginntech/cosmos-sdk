...
func bootstrapSlashTest(t *testing.T, power int64) (bankkeeper.Keeper, *keeper.Keeper, authkeeper.AccountKeeper, sdk.Context, []sdk.AccAddress, []sdk.ValAddress) {
	var (
	...
	addrDels, addrVals := generateAddresses(bankKeeper, stakingKeeper, ctx, 100)
...
func TestRevocation(t *testing.T) {
	_, stakingKeeper, _, ctx, _, addrVals := bootstrapSlashTest(t, 5)
...
func TestSlashUnbondingDelegation(t *testing.T) {
	bankKeeper, stakingKeeper, _, ctx, addrDels, addrVals := bootstrapSlashTest(t, 10)
...
func TestSlashRedelegation(t *testing.T) {
	bankKeeper, stakingKeeper, accountKeeper, ctx, addrDels, addrVals := bootstrapSlashTest(t, 10)
	fraction := sdk.NewDecWithPrec(5, 1)
...
func TestSlashAtFutureHeight(t *testing.T) {
	_, stakingKeeper, _, ctx, _, _ := bootstrapSlashTest(t, 10)

	consAddr := sdk.ConsAddress(PKs[0].Address())
	fraction := sdk.NewDecWithPrec(5, 1)
	require.Panics(t, func() { stakingKeeper.Slash(ctx, consAddr, 1, 10, fraction) })
}
...
func TestSlashAtNegativeHeight(t *testing.T) {
	bankKeeper, stakingKeeper, _, ctx, _, _ := bootstrapSlashTest(t, 10)
	consAddr := sdk.ConsAddress(PKs[0].Address())
	fraction := sdk.NewDecWithPrec(5, 1)
...
func TestSlashWithRedelegation(t *testing.T) {
	...
	require.NotPanics(t, func() { stakingKeeper.Slash(ctx, consAddr, 10, 10, fraction) })
	...
	require.NotPanics(t, func() { stakingKeeper.Slash(ctx, consAddr, 10, 10, sdk.OneDec()) })
...
func TestSlashAmount(t *testing.T) {
	bankKeeper, stakingKeeper, _, ctx, _, addrVals := bootstrapSlashTest(t, 10)
	consAddr := sdk.ConsAddress(PKs[0].Address())
	fraction := sdk.NewDecWithPrec(5, 1)
	...
	_, addrVals = generateAddresses(bankKeeper, stakingKeeper, ctx, 100)
