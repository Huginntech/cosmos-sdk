...
func bootstrapValidatorTest(t testing.TB, power int64, numAddrs int) (bankkeeper.Keeper, *keeper.Keeper, authkeeper.AccountKeeper, sdk.Context, []sdk.AccAddress, []sdk.ValAddress) {
	...
	addrDels, addrVals := generateAddresses(bankKeeper, stakingKeeper, ctx, numAddrs)
...
func TestUpdateValidatorByPowerIndex(t *testing.T) {
	...
	_, addrVals := generateAddresses(bankKeeper, stakingKeeper, ctx, 1)
...
func TestValidatorBasics(t *testing.T) {
	...
	assert.Equal(t, stakingKeeper.TokensFromConsensusPower(ctx, 9), validators[0].Tokens)
	assert.Equal(t, stakingKeeper.TokensFromConsensusPower(ctx, 8), validators[1].Tokens)
	assert.Equal(t, stakingKeeper.TokensFromConsensusPower(ctx, 7), validators[2].Tokens)
	...
	assert.True(sdk.IntEq(t, stakingKeeper.TokensFromConsensusPower(ctx, 9), validators[0].BondedTokens()))
